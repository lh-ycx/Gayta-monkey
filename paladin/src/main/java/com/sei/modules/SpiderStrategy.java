...
public class SpiderStrategy implements Strategy{
    ...
    List<Integer> contentHash = new ArrayList<Integer>();       //记录已经爬到的页面的内容hash
    ...
    public Decision make(String serial, ViewTree currentTree, ViewTree newTree, Decision prev_decision, int response){
        ...
        if (newTree.getActivityName().equals(device.targetActivity)) {
            ...
            //比较content hash。
            int tempHash = newTree.calContentHash();
            //CommonUtil.log("tempHash" + tempHash);
            if (!contentHash.contains(tempHash)) {
                //记录并截图
                CommonUtil.SPIDERMODE = true;
                CommonUtil.getSnapshot(newTree, device);
                CommonUtil.SPIDERMODE = false;
                contentHash.add(tempHash);
            } else {
                CommonUtil.log("old content:" + newTree.getActivityName() + "_" + newTree.getTreeStructureHash() + "_" + tempHash);
            }
            //不继续点击，回到他的父页面。
