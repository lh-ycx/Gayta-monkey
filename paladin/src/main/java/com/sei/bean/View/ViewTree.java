...
//        display(root, 0);
    ...
    public int calContentHash() {
        int contentHash = 1;
        String content = "";
        Queue<ViewNode> queue = new LinkedList<ViewNode>();
        queue.offer(this.root);
        while(!queue.isEmpty()) {
            ViewNode tmpNode = queue.poll();
            content += tmpNode.getViewText();
            for(ViewNode t: tmpNode.children) {
                queue.offer(t);
            }
        }
        //log("content：" + content);
